...
vec3 get_normal()
{
    vec3 tangent_normal = texture(normal_map, fs_in.tex_coord).xyz * 2.0 - 1.0;
...
void main()
{
	...
	int y = (block_size - 1) - int(gl_FragCoord.y / block_size_y);
	...
	{ //Dir light
		vec3 V = normalize(cam_pos.xyz - fs_in.world_pos);
