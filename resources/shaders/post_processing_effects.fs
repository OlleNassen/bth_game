...
//float width = scene_textureWidth; //texture width
//float height = scene_textureHeight; //texture height
...
float ndofstart = 0.5; //near dof blur start
...
void main() 
{
	...
	if (depthblur)
	{
		depth = linearize(bdepth(tex_coord.xy));
	}
	...
	//gl_FragColor.rgb = texture(scene_texture, texcoord);
	frag_color.rgb = col;
	frag_color.a = 1.0;
}